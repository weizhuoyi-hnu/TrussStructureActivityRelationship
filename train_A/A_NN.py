...
hidden_layer = [1500, 3000, 3000, 3000, 1500]
learning_rate = 1e-4
batch_size = 1280
train_epoch = 200
...
label_scaled = norm_label_A(label)
...
print('---------------------------')
torch.save(model, 'F:/wzy/TrussStructureActivityRelationship/model/NN/A/neural_network_A.pt')
pd.DataFrame(data=train_history, index=None, columns=['train_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/A/train_loss_A.csv')
pd.DataFrame(data=test_history, index=None, columns=['test_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/A/test_loss_A.csv')
...
with torch.no_grad():
    ...
    inv_scaled_train_label_true = inv_norm_label_A(label_train_all_np.reshape(-1, 1))
    inv_scaled_test_label_true = inv_norm_label_A(label_test_np.reshape(-1, 1))
    inv_scaled_train_label_pred = inv_norm_label_A(train_label_pred_np.reshape(-1, 1))
    inv_scaled_test_label_pred = inv_norm_label_A(test_label_pred_np.reshape(-1, 1))

    pd.DataFrame(feature_train_all_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/A/feature_A_train.csv', index=False)
    pd.DataFrame(feature_test_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/A/feature_A_test.csv', index=False)

    pd.DataFrame(inv_scaled_train_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/A/label_true_A_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/A/label_true_A_test.csv', index=False)
    pd.DataFrame(inv_scaled_train_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/A/label_pred_A_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/A/label_pred_A_test.csv', index=False)
