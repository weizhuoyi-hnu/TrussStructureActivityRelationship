...
ML_model = light_gradient_boosting(num_leaf=200, num_tree=3000, depth_tree=20, lr=0.1)
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\model_LIGHTGBM_A.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\feature_A_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\feature_A_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\label_true_A_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\label_true_A_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\label_pred_A_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\A\label_pred_A_test.csv', index=False)
