...
param_grid = {
    'n_estimators': [6000, 7000, 8000, 9000, 10000],
    'max_depth': [3, 4, 5, 6, 7],
...
for i, (n_est, max_d, lr) in enumerate(param_combinations):
    print(f"\n[{i+1}/{len(param_combinations)}] Training model with: "
          f"n_estimators={n_est}, max_depth={max_d}, learning_rate={lr}")
...
df_results.to_csv(r'F:\wzy\TrussStructureActivityRelationship\result\XGBOOST\ey\manual_gridsearch_results.csv', index=False)
