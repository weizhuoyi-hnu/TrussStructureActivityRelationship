...
param_combinations = list(product(param_grid['num_leaves'],
...
for i, (num_leaves, max_depth, lr, n_est) in enumerate(param_combinations):
    print(f"\n[{i+1}/{len(param_combinations)}] Training model with: "
          f"num_leaves={num_leaves}, max_depth={max_depth}, "
          f"learning_rate={lr}, n_estimators={n_est}")
...
df_results.to_csv(r'F:\wzy\TrussStructureActivityRelationship\result\LIGHTGBM\ey\manual_gridsearch_results.csv', index=False)
