...
param_combinations = list(product(param_grid['n_neighbors'], param_grid['wights']))

results_list = []

for i, (n_nei, w_method) in enumerate(param_combinations):
    print(f"\n[{i+1}/{len(param_combinations)}] Training model with: "
          f"n_neighbors={n_nei}, wights={w_method}")
...
df_results.to_csv(r'F:\wzy\TrussStructureActivityRelationship\result\KNN\ey\manual_gridsearch_results.csv', index=False)
