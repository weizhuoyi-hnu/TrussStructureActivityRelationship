...
hidden_layer = [1000, 2000, 2000, 2000, 2000, 2000, 1000]
learning_rate = 1e-4
batch_size = 1280
train_epoch = 200
...
label_scaled = norm_label_ex(label)
...
print('---------------------------')
torch.save(model, 'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/neural_network_ex.pt')
pd.DataFrame(data=train_history, index=None, columns=['train_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/ex/train_loss_ex.csv')
pd.DataFrame(data=test_history, index=None, columns=['test_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/ex/test_loss_ex.csv')
...
with torch.no_grad():
    ...
    inv_scaled_train_label_true = inv_norm_label_ex(label_train_all_np.reshape(-1, 1))
    inv_scaled_test_label_true = inv_norm_label_ex(label_test_np.reshape(-1, 1))
    inv_scaled_train_label_pred = inv_norm_label_ex(train_label_pred_np.reshape(-1, 1))
    inv_scaled_test_label_pred = inv_norm_label_ex(test_label_pred_np.reshape(-1, 1))

    pd.DataFrame(feature_train_all_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/feature_ex_train.csv', index=False)
    pd.DataFrame(feature_test_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/feature_ex_test.csv', index=False)

    pd.DataFrame(inv_scaled_train_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/label_true_ex_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/label_true_ex_test.csv', index=False)
    pd.DataFrame(inv_scaled_train_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/label_pred_ex_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ex/label_pred_ex_test.csv', index=False)
