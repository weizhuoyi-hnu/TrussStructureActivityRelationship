...
Y = label_data[['ex']].reset_index(drop=True)
...
Y_train_scaled = norm_label_ex(Y_train)
Y_test_scaled = norm_label_ex(Y_test)
...
Y_train_pred = inv_norm_label_ex(Y_train_pred_scaled.reshape(-1, 1))
Y_test_pred = inv_norm_label_ex(Y_test_pred_scaled.reshape(-1, 1))
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\model_LIGHTGBM_ex.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\feature_ex_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\feature_ex_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\label_true_ex_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\label_true_ex_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\label_pred_ex_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ex\label_pred_ex_test.csv', index=False)
