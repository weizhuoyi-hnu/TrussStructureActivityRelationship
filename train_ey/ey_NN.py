...
hidden_layer = [1500, 3000, 3000, 3000, 3000, 3000, 1500]
learning_rate = 1e-4
batch_size = 1280
train_epoch = 200
...
print('---------------------------')
torch.save(model, 'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/neural_network_ey.pt')
pd.DataFrame(data=train_history, index=None, columns=['train_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/ey/train_loss_ey.csv')
pd.DataFrame(data=test_history, index=None, columns=['test_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/ey/test_loss_ey.csv')
...
with torch.no_grad():
    ...
    inv_scaled_train_label_true = inv_norm_label_ey(label_train_all_np.reshape(-1, 1))
    inv_scaled_test_label_true = inv_norm_label_ey(label_test_np.reshape(-1, 1))
    inv_scaled_train_label_pred = inv_norm_label_ey(train_label_pred_np.reshape(-1, 1))
    inv_scaled_test_label_pred = inv_norm_label_ey(test_label_pred_np.reshape(-1, 1))

    pd.DataFrame(feature_train_all_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/feature_ey_train.csv', index=False)
    pd.DataFrame(feature_test_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/feature_ey_test.csv', index=False)

    pd.DataFrame(inv_scaled_train_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/label_true_ey_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/label_true_ey_test.csv', index=False)
    pd.DataFrame(inv_scaled_train_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/label_pred_ey_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/ey/label_pred_ey_test.csv', index=False)
