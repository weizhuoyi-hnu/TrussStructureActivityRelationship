...
Y = label_data[['ey']].reset_index(drop=True)
...
Y_train_pred = inv_norm_label_ey(Y_train_pred_scaled.reshape(-1, 1))
Y_test_pred = inv_norm_label_ey(Y_test_pred_scaled.reshape(-1, 1))
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\model_LIGHTGBM_ey.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\feature_ey_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\feature_ey_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\label_true_ey_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\label_true_ey_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\label_pred_ey_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\ey\label_pred_ey_test.csv', index=False)
