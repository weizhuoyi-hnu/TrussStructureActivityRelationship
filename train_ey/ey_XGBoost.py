...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\model_XGBOOST_ey.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\feature_ey_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\feature_ey_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\label_true_ey_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\label_true_ey_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\label_pred_ey_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\XGBOOST\ey\label_pred_ey_test.csv', index=False)
