...
ML_model = k_nearest_neighbor(num_neighbor=5, method='distance')
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\model_KNN_vyx.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\feature_vyx_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\feature_vyx_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\label_true_vyx_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\label_true_vyx_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\label_pred_vyx_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\KNN\vyx\label_pred_vyx_test.csv', index=False)
