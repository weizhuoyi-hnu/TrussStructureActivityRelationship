...
ML_model = random_forest(num_tree=300, depth_tree=30)
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\model_RF_vyx.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\feature_vyx_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\feature_vyx_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\label_true_vyx_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\label_true_vyx_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\label_pred_vyx_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\RF\vyx\label_pred_vyx_test.csv', index=False)
