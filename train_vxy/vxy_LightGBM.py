...
Y = label_data[['vxy']].reset_index(drop=True)
...
Y_train_scaled = norm_label_vxy(Y_train)
Y_test_scaled = norm_label_vxy(Y_test)
X_train_scaled = X_train_scaled.astype(np.float32)
X_test_scaled = X_test_scaled.astype(np.float32)
Y_train_scaled = Y_train_scaled.astype(np.float32)
Y_test_scaled = Y_test_scaled.astype(np.float32)

ML_model = light_gradient_boosting(num_leaf=200, num_tree=3000, depth_tree=30, lr=0.1)
...
Y_train_pred = inv_norm_label_vxy(Y_train_pred_scaled.reshape(-1, 1))
Y_test_pred = inv_norm_label_vxy(Y_test_pred_scaled.reshape(-1, 1))
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\model_LIGHTGBM_vxy.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\feature_vxy_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\feature_vxy_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\label_true_vxy_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\label_true_vxy_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\label_pred_vxy_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\vxy\label_pred_vxy_test.csv', index=False)
