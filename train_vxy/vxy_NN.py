...
hidden_layer = [1500, 3000, 3000, 3000, 3000, 1500]
learning_rate = 1e-4
batch_size = 1280
train_epoch = 200
...
label_scaled = norm_label_vxy(label)
...
print('---------------------------')
torch.save(model, 'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/neural_network_vxy.pt')
pd.DataFrame(data=train_history, index=None, columns=['train_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/train_loss_vxy.csv')
pd.DataFrame(data=test_history, index=None, columns=['test_loss']).to_csv('F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/test_loss_vxy.csv')
...
with torch.no_grad():
    ...
    inv_scaled_train_label_true = inv_norm_label_vxy(label_train_all_np.reshape(-1, 1))
    inv_scaled_test_label_true = inv_norm_label_vxy(label_test_np.reshape(-1, 1))
    inv_scaled_train_label_pred = inv_norm_label_vxy(train_label_pred_np.reshape(-1, 1))
    inv_scaled_test_label_pred = inv_norm_label_vxy(test_label_pred_np.reshape(-1, 1))

    pd.DataFrame(feature_train_all_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/feature_vxy_train.csv', index=False)
    pd.DataFrame(feature_test_np, columns=feature_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/feature_vxy_test.csv', index=False)

    pd.DataFrame(inv_scaled_train_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/label_true_vxy_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_true, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/label_true_vxy_test.csv', index=False)
    pd.DataFrame(inv_scaled_train_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/label_pred_vxy_train.csv', index=False)
    pd.DataFrame(inv_scaled_test_label_pred, columns=label_name).to_csv(r'F:/wzy/TrussStructureActivityRelationship/model/NN/vxy/label_pred_vxy_test.csv', index=False)
