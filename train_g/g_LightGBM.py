...
Y = label_data[['g']].reset_index(drop=True)
...
Y_train_scaled = norm_label_g(Y_train)
Y_test_scaled = norm_label_g(Y_test)
...
Y_train_pred = inv_norm_label_g(Y_train_pred_scaled.reshape(-1, 1))
Y_test_pred = inv_norm_label_g(Y_test_pred_scaled.reshape(-1, 1))
...
joblib.dump(ML_model, r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\model_LIGHTGBM_g.pkl')

pd.DataFrame(X_train, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\feature_g_train.csv', index=False)
pd.DataFrame(X_test, columns=X.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\feature_g_test.csv', index=False)

pd.DataFrame(Y_train, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\label_true_g_train.csv', index=False)
pd.DataFrame(Y_test, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\label_true_g_test.csv', index=False)
pd.DataFrame(Y_train_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\label_pred_g_train.csv', index=False)
pd.DataFrame(Y_test_pred, columns=Y.columns).to_csv(r'F:\wzy\TrussStructureActivityRelationship\model\LIGHTGBM\g\label_pred_g_test.csv', index=False)
